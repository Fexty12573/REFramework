    ...
    void OnCallstackEntry(CallstackEntryType eType, CallstackEntry& entry) override {
		...
		if ((eType != lastEntry) && (entry.offset != 0))
		{
			...
			if (entry.lineFileName[0] == 0)
			{
				...
				_snprintf_s(buffer, maxLen, "%p (%s): %s: %s", entry.offset, entry.moduleName, entry.lineFileName, entry.name);
			}
			else
				_snprintf_s(buffer, maxLen, "%s (%d): %s (%p)", entry.lineFileName, entry.lineNumber, entry.name, entry.offset);
